...
sealed interface Expr permits Expr.Binary, Expr.Call, Expr.DictLiteral, Expr.Grouping, Expr.Index, Expr.ListComprehension,
        Expr.ListLiteral, Expr.Literal, Expr.MethodCall, Expr.TupleLiteral, Expr.Unary, Expr.Variable {
    ...
    // 辞書リテラル: {} や {"name": "Numa"}。
    record DictLiteral(List<DictEntry> entries) implements Expr {
    }
    ...
    // タプルリテラル: (), (1,), (1, 2)。
    record TupleLiteral(List<Expr> elements) implements Expr {
    }
